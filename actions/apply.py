...
def _build_easy_apply_locator(page: Page) -> Locator:
    """Builds the locator chain for the Easy Apply button using fallbacks."""
    ...
    for i in range(1, 4):
        fallback = selectors.get(f"easy_apply_button_fallback_{i}")
        if fallback:
            locator = locator.or_(page.locator(fallback))
...
async def _select_easy_apply_element(locator: Locator) -> Locator:
    """
    Selects the specific Easy Apply element to click from the found elements.
    Handles the logic of choosing the second element if multiple are found.
    """
    ...
    await element_locator.wait_for(state="visible")
...
async def _wait_for_easy_apply_modal(page: Page) -> None:
    ...
    try:
        # Strategy 1: Specific Easy Apply modal class (most reliable)
        try:
        ...
        # Strategy 3: Generic role="dialog" selector
        try:
            await page.wait_for_selector(
...
async def _perform_easy_apply_click(page: Page, locator: Locator, timeout: float) -> None:
    """
    ...
    # Select the specific element to click
    async with asyncio.timeout(timeout / 1000.0):
        element_locator = await _select_easy_apply_element(locator)
    ...
    await _wait_for_easy_apply_modal(page)
...
async def click_easy_apply_button(page: Page) -> None:
    ...
    # Build locator chain using .or() for automatic fallback
    locator = _build_easy_apply_locator(page)
    ...
    try:
        await executor.execute_operation(
            ...
            operation=lambda: _perform_easy_apply_click(page, locator, timeout),
