...
## Таблица `discovery_state`

- **Назначение:** Хранение состояния процесса обнаружения вакансий для watermark-based подхода. Используется для оптимизации повторных поисков.
...
| :--- | :--- | :--- |
| `id` | INTEGER, PRIMARY KEY | Автоинкремент ID записи. |
| `search_key` | TEXT, UNIQUE | Уникальный ключ поискового запроса. |
| `last_seen_max_job_id` | INTEGER | Максимальный ID вакансии из последнего поиска. |
| `last_complete_sweep_before_id` | INTEGER | ID до которого был выполнен полный проход. |
