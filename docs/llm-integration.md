...
## Генерация правил для Modal Flow

Проект поддерживает автоматическую генерацию правил для заполнения полей форм через отдельный запрос к LLM.
...
Упрощенный промпт для принятия решений о заполнении полей. Фокус только на принятии решения, без генерации правил.

#### RULE_GENERATION_PROMPT

Специализированный промпт для генерации правил:

- Инструкции по созданию regex паттернов
- Руководство по выбору стратегий для разных типов полей
- Примеры для checkbox, text, number, radio, select, combobox
- Требования к мультиязычности и качеству паттернов

### Интеграция

Генерация правил интегрирована в `RulesEngine`:

1. После принятия решения LLM вызывается `generate_rule()`
2. Правило валидируется через `RuleSuggestionValidator`
3. Если правило валидно, оно сохраняется в `RuleStore`
4. В будущем правило используется автоматически

### Конфигурация

Настройки генерации правил в `LearningConfig`:
...
learning_config = LearningConfig(
    ...
    rule_generation_fallback=True,  # Fallback на suggest_rule
